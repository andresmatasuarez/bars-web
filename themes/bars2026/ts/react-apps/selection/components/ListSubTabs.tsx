    ...
    <div className="relative mt-4">
      <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
        ...
        >
          ...
          <span
            role="button"
            tabIndex={0}
            onClick={(e) => { e.stopPropagation(); onSharePersonalList(); }}
            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.stopPropagation(); onSharePersonalList(); } }}
          ...
          <span
            role="button"
            tabIndex={0}
            onClick={(e) => { e.stopPropagation(); onSavePersonalList(); }}
            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.stopPropagation(); onSavePersonalList(); } }}
          ...
          return (
            ...
            >
              ...
              <span
                role="button"
                tabIndex={0}
                onClick={(e) => { e.stopPropagation(); requestDeleteSharedList(list.id); }}
                onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.stopPropagation(); requestDeleteSharedList(list.id); } }}
      ...
      {showTooltip && tooltipPosRef.current && (
        <span
          ...
          style={{ left: tooltipPosRef.current.x, top: tooltipPosRef.current.y - 8, transform: 'translate(-50%, -100%)' }}
        >
          Enlace copiado
        </span>
      )}
